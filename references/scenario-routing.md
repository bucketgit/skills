...
| "Agent board", "local board planning", "coordinate agents with bgit board", "multi-agent planning over s3://", "multi-agent planning over gs://" | `bgit-agent-board/SKILL.md` |
...
3. `bgit-agent-board` if the task is specifically agent planning or
   coordination through a bgit board on `file://`, `s3://`, or `gs://`.
4. `local-broker-operator` if the repository is local-broker-backed or uses
   `s3://`, `gs://`, or `file://` local broker flows.
5. `repo-starter` to establish the repository and identity.
6. `task-board-steward` or `multi-agent-coordinator` to define work.
7. `pr-ci-operator` to review, validate, and merge.
8. `broker-administrator` only when permissions, broker deployment, or repo
   policy must change.
9. `bucket-recovery-operator` only for direct bucket recovery, migration, or
